...
const modelColors = {
  "nano-banana": "text-green-400",
  "nano-banana-pro": "text-purple-400",
...
export function StyleCard({ style, index = 0 }: StyleCardProps) {
  ...
  return (
    <Link
      ...
      className={cn(
        "group relative block rounded-2xl overflow-hidden bg-[var(--card-bg)] border border-[var(--border)] card-glow",
        "transform transition-all duration-300 hover:scale-[1.02] hover:border-[var(--accent)]/50"
      )}
    ...
    >
      ...
      <div className="relative aspect-[4/5] overflow-hidden">
        ...
        {imageError ? (
          <div className="absolute inset-0 bg-gradient-to-br from-[var(--accent)]/20 to-[var(--accent-2)]/20 flex items-center justify-center">
            <div className="text-center p-4">
              <Sparkles className="w-12 h-12 text-[var(--accent)] mx-auto mb-2" />
              <span className="text-sm text-[var(--muted)]">{style.name}</span>
            </div>
          </div>
        ) : (
          <Image
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4">
          ...
          {/* Title */}
          <h3 className="text-lg font-semibold text-white mb-2 line-clamp-2 group-hover:text-[var(--accent)] transition-colors">
            {style.name}
          </h3>
